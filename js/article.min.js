...
function AppendArticle(userNumber, item, ismy, isuser, islazyload, isdel) {
	...
	if(ismy) {
		...
		if(islazyload) {
			model.push('<img class="user" userid="' + item.UserNumber + '" nickname="' + item.NickName + '"avatar="' + item.Avatar + '"cover="' + item.UserCover + '" data-lazyload="' + base.ShowThumb(item.Avatar, 1) + '" style="border-radius:50%;width:2rem !important;height:2rem !important;" />');
		} else {
		...
		model.push('<div class="mui-media-body f14 mt0" style="position:relative;"><span class="user c333" userid="' + item.UserNumber + '"nickname="' + item.NickName + '"avatar="' + item.Avatar + '"cover="' + item.UserCover + '">' + base.UnUnicodeText(item.NickName) + '</span><p class="f10 full blue mt3">');
	...
	if(parts.length == 0) {
		if(islazyload) {
			model.push('<div class="onefloor" href="' + base.ShowThumb(item.Cover, 1) + '" articleid="' + item.ArticleID + '"><img data-lazyload="' + base.ShowThumb(item.Cover, 1) + '"  data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
		} else {
			model.push('<div class="onefloor" href="' + base.ShowThumb(item.Cover, 1) + '" articleid="' + item.ArticleID + '"><img src="' + base.ShowThumb(item.Cover, 1) + '"  data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
		}
	} else {
		if(parts.length == 1) {
			if(islazyload) {
				model.push('<div class="onefloor" href="' + base.ShowThumb(parts[0].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img data-lazyload="' + base.ShowThumb(parts[0].Introduction, 1) + '" data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
			} else {
				model.push('<div class="onefloor" href="' + base.ShowThumb(parts[0].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img src="' + base.ShowThumb(parts[0].Introduction, 1) + '" data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
			}
		} else {
			var name = parts.length == 3 ? "thirdfloor" : "secondfloor";
			for(var i = 0; i < parts.length; i++) {
				if(islazyload) {
					model.push('<div class="' + name + '" href="' + base.ShowThumb(parts[i].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img data-lazyload="' + base.ShowThumb(parts[i].Introduction, 2) + '"  data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
				} else {
					model.push('<div class="' + name + '" href="' + base.ShowThumb(parts[i].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img src="' + base.ShowThumb(parts[i].Introduction, 2) + '" data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
				}
			}
		}
	}
...
function ArticleAction(id, userinfo) {
	...
	mui(id).on('tap', '.onefloor,.secondfloor,.thirdfloor', function() {
		...
		if(view) {
			base.GetView("imageviewer").evalJS("ResetImages('" + url + "','" + ArticleID + "','" + CurrID + "')");
		} else {
