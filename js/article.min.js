		...
		if(e.index == 0) {
			HttpGet(base.RootUrl + "Api/Fan/Delete", {
				...
				if(data.result) {
					...
					base.UpdateFan(userinfo, userinfo.Follows);
				} else {
					mui.toast(data.message);
				}
			});
...
function AppendArticle(userNumber, item, ismy, isuser, islazyload, isdel) {
	...
	if(ismy) {
		...
		if(islazyload) {
			model.push('<img class="user" userid="' + item.UserNumber + '" nickname="' + base.UnUnicodeText(item.NickName) + '"avatar="' + item.Avatar + '"cover="' + item.Cover + '" data-lazyload="' + base.ShowThumb(item.Avatar, 1) + '" style="border-radius:50%;width:2rem !important;height:2rem !important;" />');
		} else {
			model.push('<img class="user" userid="' + item.UserNumber + '" nickname="' + base.UnUnicodeText(item.NickName) + '"avatar="' + item.Avatar + '"cover="' + item.Cover + '"src="' + base.ShowThumb(item.Avatar, 1) + '" style="border-radius:50%;width:2rem !important;height:2rem !important;" />');
		}

		model.push('<div class="mui-media-body f14 mt0" style="position:relative;"><span class="user c333" userid="' + item.UserNumber + '"nickname="' + base.UnUnicodeText(item.NickName) + '"avatar="' + item.Avatar + '"cover="' + item.Cover + '">' + base.UnUnicodeText(item.NickName) + '</span><p class="f10 full blue mt3">');
		...
		if(userNumber != "" && userNumber != item.UserNumber) {
			model.push('<img id="guanzhu' + item.ArticleID + '" class="' + (item.IsFollow == 0 ? "guanzhu" : "") + '" userid="' + item.UserNumber + '" articleid="' + item.ArticleID + '" src="../images/base/' + (item.IsFollow == 0 ? "follow0" : "follow1") + '.png" style="position:absolute;right:0px;top:0.5rem;width:3rem;" />');
		}
	...
	if(userNumber != "" && userNumber == item.UserNumber) {
		...
		model.push('<div class="c333 fl article f12" style="width:85%;display:inline-block;" articleid="' + item.ArticleID + '" userid="' + item.UserNumber + '" power="' + item.ArticlePower + '" nickname="' + item.NickName + '">');
		if(base.IsNullOrEmpty(item.Title)) {
			item.Title = "我的小微篇";
		}
		model.push(base.UnUnicodeText(item.Title) + '</div><div class="f10 c999 fr tr" style="width:15%;display:inline-block;">' + item.CreateDate + '</div>');
	}
	model.push("</div>");
	...
	model.push('<div class="mui-card-footer fl full c333 f11" style="display:inline-block;">');
	var style = "min-width:3.5rem;height:1.5rem;display:inline-block;";

	model.push('<div style="' + style + '" class="fl tl"><img src="../images/article/view.png" style="width:1rem;" class="mr10 ml5 fl" /><span class="f13 fl">' + item.Views + '</span></div>');
	model.push('<div style="' + style + '" class="fl tl goods ' + (item.IsZan == 0 ? "" : "red") + '" articleid="' + item.ArticleID + '" already="' + (item.IsZan == 0 ? 0 : 1) + '"><img src="../images/article/' + (item.IsZan == 0 ? "btn_good" : "btn_good2") + '.png" style="width:0.9rem;" class="mr10 ml5 fl" /><span class="f13 fl">' + item.Goods + '</span></div>');
	model.push('<div style="' + style + '" class="comments fl tl" articleid="' + item.ArticleID + '" ArticleNumber="' + item.ArticleNumber + '"><img id="icomments' + item.ArticleID + '" src="../images/article/btn_comment.png" style="width:0.9rem;" class="mt1 fl mr10" /><span class="f13 fl" >' + item.Comments + '</span></div>');
	if(item.IsPay == 1) {
		model.push('<div style="' + style + '" class="pays fl tl" articleid="' + item.ArticleID + '" ArticleNumber="' + item.ArticleNumber + '" UserNumber="' + item.UserNumber + '" NickName="' + item.NickName + '" Avatar="' + item.Avatar + '" articleid="' + item.ArticleID + '"><img id="ipays' + item.ArticleID + '" src="../images/article/btn_pay.png" style="width:0.9rem;" class="mt1" /></div>');
	}
