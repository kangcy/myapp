...
function AppendArticle(userNumber, item, ismy, isuser, islazyload, isdel) {
	...
	if(ismy) {
		...
		if(islazyload) {
			model.push('<img class="user" userid="' + item.UserNumber + '" nickname="' + item.NickName + '"avatar="' + item.Avatar + '"cover="' + item.UserCover + '" onload="LazyImg(this)" src="../images/avatar.png" data-lazyload="' + base.ShowThumb(item.Avatar, 1) + '" style="border-radius:50%;width:2rem !important;height:2rem !important;" />');
		} else {
	...
	if(parts.length == 0) {
		if(islazyload) {
			model.push('<div class="onefloor" href="' + base.ShowThumb(item.Cover, 1) + '" articleid="' + item.ArticleID + '"><img onload="LazyImg(this)" src="../images/default.png" data-lazyload="' + base.ShowThumb(item.Cover, 1) + '" /></div>');
		} else {
			model.push('<div class="onefloor" href="' + base.ShowThumb(item.Cover, 1) + '" articleid="' + item.ArticleID + '"><img src="' + base.ShowThumb(item.Cover, 1) + '" /></div>');
		}
	} else {
		if(parts.length == 1) {
			if(islazyload) {
				model.push('<div class="onefloor" href="' + base.ShowThumb(parts[0].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img onload="LazyImg(this)" src="../images/default.png" data-lazyload="' + base.ShowThumb(parts[0].Introduction, 1) + '" /></div>');
			} else {
				model.push('<div class="onefloor" href="' + base.ShowThumb(parts[0].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img src="' + base.ShowThumb(parts[0].Introduction, 1) + '" /></div>');
			}
		} else {
			var name = parts.length == 3 ? "thirdfloor" : "secondfloor";
			for(var i = 0; i < parts.length; i++) {
				if(islazyload) {
					model.push('<div class="' + name + '" href="' + base.ShowThumb(parts[i].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img onload="LazyImg(this)" src="../images/default.png" data-lazyload="' + base.ShowThumb(parts[i].Introduction, 2) + '" /></div>');
				} else {
...
function ArticleAction(id, userinfo) {
	...
	mui(id).on('tap', '.tanaction', function() {
		...
		if(view) {
			view.evalJS("ResetAction(2,'" + PageName + "','" + iskeep + "','" + isfollow + "')");
		} else {
