...
function AppendArticle(userNumber, item, ismy, isuser, islazyload, isdel) {
	...
	if(parts.length == 0) {
		if(islazyload) {
			model.push('<div class="onefloor"href="' + base.ShowThumb(item.Cover, 1) + '"><img data-lazyload="' + base.ShowThumb(item.Cover, 1) + '"  data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
		} else {
			model.push('<div class="onefloor"href="' + base.ShowThumb(item.Cover, 1) + '"><img src="' + base.ShowThumb(item.Cover, 1) + '"  data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
		}
	} else {
		if(parts.length == 1) {
			if(islazyload) {
				model.push('<div class="onefloor"href="' + base.ShowThumb(parts[0].Introduction, 1) + '"><img data-lazyload="' + base.ShowThumb(parts[0].Introduction, 1) + '" data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
			} else {
				model.push('<div class="onefloor"href="' + base.ShowThumb(parts[0].Introduction, 1) + '"><img src="' + base.ShowThumb(parts[0].Introduction, 1) + '" data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
			}
		} else {
			var name = parts.length == 3 ? "thirdfloor" : "secondfloor";
			for(var i = 0; i < parts.length; i++) {
				if(islazyload) {
					model.push('<div class="' + name + '"href="' + base.ShowThumb(parts[i].Introduction, 1) + '"><img data-lazyload="' + base.ShowThumb(parts[i].Introduction, 2) + '"  data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
				} else {
					model.push('<div class="' + name + '"href="' + base.ShowThumb(parts[i].Introduction, 1) + '"><img src="' + base.ShowThumb(parts[i].Introduction, 2) + '" data-preview-src="" data-preview-group="' + item.ArticleID + '" /></div>');
				}
			}
		}
	}
...
function ArticleAction(id, userinfo) {
	...
	mui(id).on('tap', '.onefloor,.secondfloor,.thirdfloor', function() {
		...
		base.OpenWindow("imageview", "imageview.html", {
			Url: url.join(',')
		}, "fade-in", false, 'hide');
	});
}
