...
function AppendArticle(userNumber, item, ismy, isuser, islazyload, isdel) {
	...
	if(ismy) {
		...
		model.push('<div class="mui-media-body f14 mt0" style="position:relative;"><span class="user c333 wave" userid="' + item.UserNumber + '"nickname="' + item.NickName + '"avatar="' + item.Avatar + '"cover="' + item.UserCover + '">' + base.UnUnicodeText(item.NickName) + '</span><p class="f10 full blue mt3">');
	...
	if(parts.length == 0) {
		if(islazyload) {
			model.push('<div class="onefloor" href="' + base.ShowThumb(item.Cover, 1) + '" articleid="' + item.ArticleID + '"><img data-lazyload="' + base.ShowThumb(item.Cover, 1) + '" /></div>');
		} else {
		...
		if(parts.length == 1) {
			if(islazyload) {
				model.push('<div class="onefloor" href="' + base.ShowThumb(parts[0].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img data-lazyload="' + base.ShowThumb(parts[0].Introduction, 1) + '" /></div>');
			} else {
			...
			for(var i = 0; i < parts.length; i++) {
				if(islazyload) {
					model.push('<div class="' + name + '" href="' + base.ShowThumb(parts[i].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img data-lazyload="' + base.ShowThumb(parts[i].Introduction, 2) + '" /></div>');
				} else {
	...
	mui(id).on('tap', '.onefloor,.secondfloor,.thirdfloor', function() {
		...
		if(view) {
			...
			base.OpenWindow("imageviewer", "imageviewer.html", {
				...
				Group: ArticleID,
				CurrID: CurrID
			}, "fade-in", false, 'hide', "transparent");
		}
		isLoading = false;
	});
