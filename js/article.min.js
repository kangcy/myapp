...
function AppendArticle(userNumber, item, ismy, isuser, islazyload, isdel) {
	...
	if(ismy) {
		...
		if(userNumber != "" && userNumber != item.UserNumber) {
			model.push('<img id="guanzhu' + item.ArticleID + '" class="' + (item.IsFollow == 0 ? "guanzhu" : "guanzhu2") + '" userid="' + item.UserNumber + '" articleid="' + item.ArticleID + '" articleNumber="' + item.ArticleNumber + '" articleUserNumber="' + item.UserNumber + '" src="../images/base/' + (item.IsFollow == 0 ? "follow0" : "follow1") + '.png" style="position:absolute;right:0px;top:0.5rem;width:3rem;" />');
		}
	...
	if(parts.length == 0) {
		if(islazyload) {
			model.push('<div class="onefloor" href="' + base.ShowThumb(item.Cover, 1) + '" articleid="' + item.ArticleID + '"><img onload="LazyImg(this)" src="../images/default.png" data-lazyload="' + base.ShowThumb(item.Cover, 2) + '" /></div>');
		} else {
		...
		if(parts.length == 1) {
			if(islazyload) {
				model.push('<div class="onefloor" href="' + base.ShowThumb(parts[0].Introduction, 1) + '" articleid="' + item.ArticleID + '"><img onload="LazyImg(this)" src="../images/default.png" data-lazyload="' + base.ShowThumb(parts[0].Introduction, 2) + '" /></div>');
			} else {
