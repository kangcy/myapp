...
var base = new function() {
	...
	this.AppendUser = function(item, isSignature, isFollow, isDelete, deleteName, isDistance, isLazy, showAnimate) {
		...
		if(isLazy) {
			model.push('<div class="avatar"><img onload="LazyImg(this)" src="../images/avatar.png" data-lazyload="' + base.ShowThumb(item.Avatar, 1) + '" class="fl"  /></div>');
		} else {
		...
		model.push('<p class="f13 c333">' + base.UnUnicodeText(item.NickName) + '</p>');
		if(isSignature) {
...
function LazyImg(item) {
	...
	item.setAttribute("src", item.getAttribute("data-lazyload"));
}
