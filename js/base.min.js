...
var base = new function() {
	...
	this.AppendUser = function(item, isSignature, isFollow, isDelete, deleteName, isDistance, isLazy, showAnimate) {
		...
		div.setAttribute("userid", item.Number);
		div.setAttribute("nickname", base.UnUnicodeText(item.NickName));
		...
		//头像
		model.push('<div style="flex:0 0 18%;" class="flex-item tl">');
		if(isLazy) {
			model.push('<div class="avatar"><img data-lazyload="' + base.ShowThumb(item.Avatar, 1) + '"  /></div>');
		} else {
			model.push('<div class="avatar"><img src="' + base.ShowThumb(item.Avatar, 1) + '"  /></div>');
		}
		model.push('</div>');
		//昵称信息
		if(isFollow)
			model.push('<div style="flex:0 0 62%;" class="flex-item tl">');
		else
			model.push('<div style="flex:0 0 82%;" class="flex-item">');
		...
		if(isFollow) {
			...
			if(item.IsFollow == 0 && userinfo.Number != item.Number) {
				model.push('<div class="guanzhu" userid="' + item.Number + '"><img src="../images/base/follow0.png" /></div>');
			} else {
				model.push('<div class="guanzhu2"><img src="../images/base/follow1.png" /></div>');
			}
			model.push('</div>');
		}
		model.push('</div>');
