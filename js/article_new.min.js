...
function OutFollow() {
	...
	mask.close();
	mui.confirm('确定要取消关注吗？', '', ['确定', '取消'], function(e) {
		if(e.index < 0) {
			return;
		}
		if(e.index == 0) {
...
function ArticleAddComment(id, userinfo) {
	mui(id).on('tap', '.comments', function(event) {
		...
		base.OpenWindow("articleComment", "../page/articleComment.html", param);
		//base.ShowTemplate("articleComment", "../page/articleComment.html", "评论", JSON.stringify(param));
	});
}